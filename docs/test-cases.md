...
2. Run: `java -cp out -Dfile.encoding=UTF-8 hok.Main`
...
| **Actual Output** | "Hero not found: Luban" — exact name match required |
| **Result** | FAIL |
| **Bug Found** | Hero search requires exact name match, not partial match |
...
| 6 | TC-06 | Hero Details — With Owners | FAIL |
...
### Issues Found
1. **TC-06**: Hero search requires exact name match, not partial match. Searching for "Luban" does not find "Luban No.7".
2. **TC-11**: Minor display formatting issue with % sign placement in the leaderboard.
